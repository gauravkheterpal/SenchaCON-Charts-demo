...
Ext.define('Ext.ux.Fileup', {
    ...
    onChanged: function(e) {
        ...
        if (e.target.files.length > 0) {
        	...
        	if (me.checkFileExtension(file)) {
        		if (me.checkFileSize(file)) {
        			me.fireAction('ready', [file], function() {
                        me.changeState('ready');
                        ReplayAnalytics.app.getApplication().getController('Admin').setSelectedFileName(file.name);
                    }, me);
        		} else {
        			Ext.Msg.alert('ReplayAnalytics &#153;', 'Selected file size is greater than 2MB. Please select a smaller file.', function(){
        				me.changeState('browse');
        			});
        		}
        	} else {
        		Ext.Msg.alert('ReplayAnalytics &#153;', 'Selected file is not a MS Excel document. Please select a vaild file.', function(){
    				me.changeState('browse');
    			});
        	}            
        } else {
        	...
        	Ext.Msg.alert('Error', 'File selected but not accessible', function(){
				me.changeState('browse');
			});
        }
    },
    ...
    checkFileExtension: function(file) {
    	...
    	if (extension != undefined && extension.length > 1){
    ...
    checkFileSize: function(file) {
    	...
    	if (fileSize/1024 < 2000) { // File size is greater than 2000KB (2MB).
    		return true;
    	}
    	return false;
    },
